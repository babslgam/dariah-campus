...
The following sections try to give an insight into the benefits of regression testing using the example of migrating a digital edition[^1] to a slightly different tech stack. In this scenario e2e testing is used to make sure that the new application displays the data after the migration exactly as it did before.
...
A single test checking the correctness of a short info:
  ...
  Test steps for short info on person entity[^2]
  </Figure>

In around 2 minutes 914 tests have been run, demonstrating the high efficiency of the provided setup.

## Conclusion

The provided resource demonstrated the use and benefits of e2e-testing in the field of digital humanities through the example of a digital edition.
Integrating tests not only reduces development time drastically but also prevents regressions and builds a solid foundation for refactoring processes. 

---
[^1]: [https://github.com/karl-kraus/wpn-static](https://github.com/karl-kraus/wpn-static)
[^2]: the difference between 914 run tests and 899 annotations arises because a annotated string can refer to more than one entity.
